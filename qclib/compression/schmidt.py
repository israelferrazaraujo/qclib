...
"""
Implements the quantum state compressor
defined at .
"""
...
from qclib.gates.compressor import Compressor
...
class SchmidtCompressor(Compressor):
    ...
    def __init__(self, params, label=None, opt_params=None):
        ...
        # The reset and output qubits must take into account that the qiskit qubits are reversed.
        complement = sorted(
        ...
        self.reset_qubits = set(range(self.num_qubits)).difference(set(self.channel_qubits))
    ...
    def _define_initialize(self):
        ...
        if self.num_qubits < 2:
            return TopDownInitialize(self.params).definition.inverse()
    ...
    def initialize(q_circuit, state, qubits=None, opt_params=None):
        ...
        if qubits is None:
            q_circuit.append(
                SchmidtCompressor(state, opt_params=opt_params), q_circuit.qubits
            )
        else:
            q_circuit.append(SchmidtCompressor(state, opt_params=opt_params), qubits)
